...
class Trainer():
    def __init__(self, args):
        ...
        self.result_dir = "./checkpoints"
        if not os.path.isdir(self.result_dir):
            os.mkdir(self.result_dir)
        self.results_filename = '{}'.format("record_metrics")    # new from yi to record metrics
    ...
    def train_loop(self):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            prediction, f = self.model(image)    # new line
            #loss_value = self.loss_func(prediction, gt)
            loss_value = self.loss_func(prediction, gt, f)    # new line
            loss_value.backward()
            self.optimizer.step()
    ...
    def val_loop(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.val_loader):
                ...
                inv_prediction, f = self.model(image)    # new line
                prediction = self.inverse_depth_norm(inv_prediction)    # new line
                ...
                loss_value = self.loss_func(inv_prediction, self.depth_norm(gt), f)
                accumulated_loss += loss_value.item()
        ...
        self.save_results(avg)    # new from yi to record metrics
    ...
    def save_results(self, average):    # new from yi to record metrics
        ...
        with open(file_path, 'a') as f:
            ...
            f.write('{average.rmse:.3f}'
             ...
             ',{average.delta3:.3f}'
             ',{epoch}\n'.format(
	         average=average, epoch=self.epoch + 1))
